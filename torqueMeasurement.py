...
class TorqueMeasurement:
    ...
    def icAngleFit(obj, bool2Plot):
        ...
        p0 = [1, 3, 1, 12, 0, 0]  # [i01, gamma1, i02, gamma2, angle_offset1, angle_offset2]
        lower_bounds = [0, 1, 0, 1, -np.pi/2, -np.pi/2]
        upper_bounds = [np.inf, 5, np.inf, 15, np.pi/2, np.pi/2]

        # Fit model to data
        i0sGammas, _ = curve_fit(doubleLorentz, x, y, p0=p0, bounds=(lower_bounds, upper_bounds), method='trf', max_nfev=1000)
